...
export function getOrCreateVaultTransfer(id: string, createIfNotFound = true): Transfer {
...
export function getOrCreateVaultDeposit(id: string, createIfNotFound = true): Deposit {
  // @ts-ignore: assign wrapper object to primitive
...
export function getOrCreateVaultWithdrawal(id: string, createIfNotFound = true): Withdrawal {
  // @ts-ignore: assign wrapper object to primitive
...
export function getOrCreateHarvest(id: string, createIfNotFound = true): Harvest {
  // @ts-ignore: assign wrapper object to primitive
...
export function getOrCreateController(address: Address, createIfNotFound = true): Controller {
...
export function getOrCreateStrategy(address: Address, createIfNotFound = true): Strategy {
  let id = address.toHexString();
...
export function getOrCreateTransaction(id: string, createIfNotFound = true): Transaction {
  // @ts-ignore: assign wrapper object to primitive
...
export function getOrCreateVault(vaultAddress: Address, update = true): Vault {
  ...
  if (update) {
    ...
    vault.pricePerFullShareRaw = !pricePerFullShare.reverted ? pricePerFullShare.value : vault.pricePerFullShareRaw;
    vault.totalSupplyRaw = !totalSupply.reverted ? totalSupply.value : vault.totalSupplyRaw;
    ...
    if (!controllerAddress.reverted) {
      ...
      if (!wrappedVaultAddress.reverted) {
        ...
        controllerContract = ControllerContract.bind(Address.fromString(wrappedVault.currentController));
      } else {
        log.critical("Vault doesn't have a controller nor a wrapped vault. Vault ID: {}", [vault.id]);
      }
    }
    ...
    vault.strategyBalanceRaw = !strategyBalance.reverted ? strategyBalance.value : vault.strategyBalanceRaw;
